...
namespace Baker76.Pngcs
{
    ...
    internal class PngIDatChunkInputStream : Stream
    {
        ...
        private readonly Baker76.Pngcs.Zlib.CRC32 crcEngine;
        ...
        public PngIDatChunkInputStream(Stream iStream, int lenFirstChunk, long offset_0)
        {
            ...
            // we know it's a IDAT
            System.Array.Copy((Array)(Baker76.Pngcs.Chunks.ChunkHelper.b_IDAT), 0, (Array)(idLastChunk), 0, 4);
        ...
        private void EndChunkGoForNext()
        {
            ...
            do
            {
                int crc = Baker76.Pngcs.PngHelperInternal.ReadInt4(inputStream); //
                offset += 4;
                if (checkCrc)
                {
                ...
                lenLastChunk = Baker76.Pngcs.PngHelperInternal.ReadInt4(inputStream);
                if (lenLastChunk < 0)
                    throw new PngjInputException("invalid len for chunk: " + lenLastChunk);
                toReadThisChunk = lenLastChunk;
                Baker76.Pngcs.PngHelperInternal.ReadBytes(inputStream, idLastChunk, 0, 4);
                offset += 8;

                ended = !PngCsUtils.arraysEqual4(idLastChunk, Baker76.Pngcs.Chunks.ChunkHelper.b_IDAT);
                if (!ended)
                {
        ...
        public void ForceChunkEnd()
        {
            if (!ended)
            {
                ...
                Baker76.Pngcs.PngHelperInternal.ReadBytes(inputStream, dummy, 0, toReadThisChunk);
                if (checkCrc)
