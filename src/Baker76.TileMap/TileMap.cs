...
namespace Baker76.TileMap
{
    ...
    public class TileMap
    {
        ...
        public static void CreateTiledTmx(string srcImagePath, TileMapSliceOptions options)
        {
            ...
            List<Image> uniqueTiles = new List<Image>();
            Dictionary<ulong, TiledNode> tileDictionary = new Dictionary<ulong, TiledNode>();
            int tileCount = 0;
            int bitsPerPixel = ((srcImage.BitsPerPixel <= 8) ? 8 : 32);
            if (options.InsertBlankTile)
            {
            ...
            for (int y = 0; y < tileRows; y++)
            {
                for (int x = 0; x < tileCols; x++)
                {
                    ...
                    if (!tileDictionary.ContainsKey(hash))
                    {
                        ...
                        if (options.NoMirror)
                        {
                            ulong noMirrorHash = tileImage.GetHash(ImageAttributes.MirrorX);
                            if (!tileDictionary.ContainsKey(noMirrorHash))
                            {
                        ...
                        if (options.NoRotate)
                        {
                            ulong noRotateHash = tileImage.GetHash(ImageAttributes.MirrorX_Y | ImageAttributes.Rotate);
                            if (!tileDictionary.ContainsKey(noRotateHash))
                            {
                            ...
                            if (!tileDictionary.ContainsKey(noRotateHash))
                            {
                                tileDictionary.Add(noRotateHash, new TiledNode(id, TiledAttributes.Horizontal_Vertical | TiledAttributes.AntiDiagonal));
                            }
                            noRotateHash = tileImage.GetHash(ImageAttributes.Rotate | ImageAttributes.MirrorY);
                            if (!tileDictionary.ContainsKey(noRotateHash))
                            {
